...
var showStats = function(inputNum, theArray){

  var infoObj = {
    count_inputGreaterThan: 0,
    count_inputLessThan: 0,
    count_inputEqualTo: 0,
    setSize: theArray.length
  }

  for(var i = 0; i < theArray.length; i++){
    if(inputNum === theArray[i]){
      infoObj.count_inputEqualTo = infoObj.count_inputEqualTo + 1
    }else if(inputNum < theArray[i]){
      infoObj.count_inputLessThan = infoObj.count_inputLessThan + 1
    }else{
      infoObj.count_inputGreaterThan = infoObj.count_inputGreaterThan + 1
    }
  }
  return infoObj
}
